        ...
        while (count > 0) {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    ...
                    //ycf-start增加请求类型参数
                    try {
                        if (!map.containsKey("properties.dl")) {
                            if (ev.equals("$AppViewScreen") || ev.equals("$pageview")) {
                                map.put("WT.dl", "0");
                            } else if (ev.equals("$AppClick") || ev.equals("$WebClick")) {
                                map.put("WT.dl", "25");
                            } else if (ev.equals("AppCrashed")) {
                                map.put("WT.dl", "99");
                            } else if (ev.equals("$AppStart")) {
                                map.put("WT.dl", "22");
                            } else if (ev.equals("$AppEnd")) {
                                map.put("WT.dl", "23");
                            } else {
                                map.put("WT.dl", "21");
                            }
                        } else {
                            map.put("WT.dl", map.get("properties.dl"));
                        }
                    }catch (Exception  e){
                    ...
                    //ycf-start新增WT.co_f访客识别参数及页面title
                    try {
                        if (map.containsKey("distinct_id")) {
                            String distinct_id = map.get("distinct_id");
                            map.put("WT.co_f", distinct_id);
                        }
                        if (map.containsKey("properties.$title")) {
                            String title = map.get("properties.$title");
                            map.put("WT.ti", title);
                        } else {
                            String title = SensorsDataAPI.sharedInstance(mContext).getActivityTitle();
                            map.put("properties.$title", title);
                            map.put("WT.ti", title);
                        }
                    }catch(Exception e){
                            SALog.printStackTrace(e);
