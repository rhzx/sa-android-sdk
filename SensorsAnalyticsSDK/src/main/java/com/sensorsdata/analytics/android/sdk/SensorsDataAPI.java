...
public class SensorsDataAPI implements ISensorsDataAPI {
    ...
    public enum ServerMode {
        SENSORS_SERVER,SDC_SERVER,FLUME_SERVER;
    }
    ...
    SensorsDataAPI(Context context, String serverURL, DebugMode debugMode) {
         ...
         * 郑卓源增加代码
         */
   /*     if (mDeviceInfo.containsKey("$device_id")&&mDeviceInfo.get("$device_id").equals("100fff560f6bc227"))
        {
            mDebugMode = DebugMode.DEBUG_AND_TRACK;
            enableLog(true);
        }//郑卓源增加代码
        */
    }
    ...
    public void setServerUrl(String serverUrl) {
        try {
             ...
             * //郑卓源增加
             */
            if(serverUrl.contains("/events.svc")) {
                mServerUrl = serverUrl;
                serverMode = ServerMode.SDC_SERVER;
            }//郑卓源增加
            else if (TextUtils.isEmpty(serverUrl) || mDebugMode == DebugMode.DEBUG_OFF) {
                mServerUrl = serverUrl;
                serverMode = ServerMode.SENSORS_SERVER;//郑卓源增加
            } else {
    ...
    public void trackEventFromH5(String eventInfo) {
        try {
            ...
            if (eventType.isTrack()) {
                ...
                //ycf-starth5页面返回记录中新增$screen_sr
                if(!propertiesObject.has("$screen_sr")){
                    if(propertiesObject.has("$screen_width") && propertiesObject.has("$screen_height")){
                        propertiesObject.put("$screen_sr",propertiesObject.get("$screen_height")+"x"+propertiesObject.get("$screen_width"));
                    }
                }
                //ycf-end
        ...
        try {
            ...
            try{
                if(properties.has("$title")){
                    mActivityTitle = properties.optString("$title");
                }
            }catch (Exception e){
                com.sensorsdata.analytics.android.sdk.SALog.printStackTrace(e);
            }
            ...
            try {
                ...
                if (eventType.isTrack()) {
                    ...
                    //ycf-start新增分辨率$screen_sr
                    if(sendProperties.has("$screen_height") && sendProperties.has("$screen_width"))
                    {
                        String mSrcHeight = sendProperties.getString("$screen_height");
                        String mSrcWidth = sendProperties.getString("$screen_width");
                        sendProperties.put("$screen_sr",mSrcHeight+"x"+mSrcWidth);
                    }
                    //ycf-end
    ...
    // SDK版本
    static final String VERSION = "3.0.3a";
  ...
  /* SensorsAnalytics 地址 */
    ...
    public ServerMode getServerMode(){return serverMode;}//郑卓源
     ...
     * 郑卓源增加
     */
    /*数据已经上报但服务器没有返回任何信息时是否重发*/
    private boolean mIsResendWhenNoResponse = false;
    public boolean getIsResendWhenNoResponse() {return this.mIsResendWhenNoResponse;}//郑卓源
